...
public:
	...
	{
		m_Cardinal.push_back(Vec2i(0, 1));  //North
		m_Cardinal.push_back(Vec2i(1, 0));	//East
		m_Cardinal.push_back(Vec2i(0, -1));	//South
		m_Cardinal.push_back(Vec2i(-1, 0));	//West	
...
private:
	...
	void Carve(Vec2i pos, TILE tile); //Carves a tile into a new tile type
	void AddRooms(); //Places rooms throughout dungeon
	void GrowMaze(Vec2i);  //Spanning tree algorithm to create a maze between all of the rooms that get created
	bool CanCarve(Vec2i, Vec2i); //given a position and a direction, returns if you can carve in that direction
	void connectRegions(); //connects the different sections of the maze together
	void AddJunction(Vec2i); //creates a door at the given position
	...
	TerrainTile MakeWall(Vec2i);	//Helper function to decide which wall sprite to use
	TerrainTile MakeDoor(Vec2i);	//Helper function to decide which door sprite to use
	TerrainTile MakeFloor(Vec2i);	//Helper function to decide which floor sprite to use
