...
void TerrainObject_Manager::SpawnObject(AVec2i Pos, PICKUP_TYPES Type)
{
...
void TerrainObject_Manager::SpawnObjectRandom(AVec2i Pos, int ChanceToSpawn)
{
	...
	{
		SpawnObject(AVec2i(Pos.x(), Pos.y()), FOOD);
	}
	else if (RandNum < ChanceForFood + ChanceForSpeed)
	{
		SpawnObject(AVec2i(Pos.x(), Pos.y()), SPEED);
	}
	else
	{
		SpawnObject(AVec2i(Pos.x(), Pos.y()), STR);
	}
}
